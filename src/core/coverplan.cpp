...
// the same.
//
// The plan lives here because three renderers draw it: the Windows viewer
// paints it with Direct2D so it can move with the music, the thumbnail pipeline
// rasterises the resting pose once on a worker thread, and the Wayland build
// does both on the processor. None of them is allowed to disagree about what
// the picture is, so none of them owns the maths.
#include "coverplan.h"
...
uint64_t coverSeedBytes(const void* bytes, size_t n) {
    ...
    for (size_t i = 0; i < n; ++i) {
        uint8_t c = p[i];
        if (c >= 'A' && c <= 'Z') c = (uint8_t)(c - 'A' + 'a');   // ASCII fold, as before
        h ^= (uint64_t)c;
        h *= 1099511628211ull;
    }
    return h ? h : 1;
...
uint64_t coverSeed(const char* utf8Name) {
    return coverSeedBytes(utf8Name, utf8Name ? strlen(utf8Name) : 0);
}
...
// Direct2D version uses. Every renderer reads from this so they agree.
